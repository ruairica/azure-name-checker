...
const Rules: React.FC<ResourceRules> = ( resource: ResourceRules ) => {
  return (
<div className="bg-blue-100 border-l-4 mt-3 border-blue-500 text-blue-700 p-4 rounded-md shadow-md" role="alert">
  <ul className="list-disc list-inside text-base">
    {Object.entries(resource)
      .filter(([k, _]) => ResourceRuleKeys.includes(k as keyof ResourceRules))
      .map(([key, value]) => {
        if (!!value) {
          return (
           <li key={key} className="mb-2 flex">
              <span className="font-semibold mr-2">{propertyLabels[key]}:</span>
